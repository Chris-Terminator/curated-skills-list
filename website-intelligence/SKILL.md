...
that the Firecrawl skill or MCP is available and authenticated.

### Check if Firecrawl Skill or MCP is connected

Look for Firecrawl tools in your available MCP tools (e.g., `mcp__firecrawl__scrape`,
`mcp__firecrawl__map`, `mcp__firecrawl__search`) OR references in the Firecrawl skill. If they are available, you're good to go.
...
1. **If using Claude Code (or any AI coding tool) with MCP**: The user needs to add Firecrawl as an MCP server in their settings. Guide them to add it via the settings UI or add it yourself:
   ```json
   {
     "mcpServers": {
       "firecrawl": {
...
2. **If Firecrawl MCP or Skill is not an option**: Fall back to using `WebFetch` for basic page scraping.
